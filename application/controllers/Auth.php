...
class Auth extends CI_Controller {
	...
	private function _sendEmail($token, $type)
    {
        ...
        'smtp_user' => 'mariaanjelin27@gmail.com',
        'smtp_pass' => 'bmgvjalzebaxqhxa',
        'smtp_port' => 465,
        'mailtype' => 'html',
        'charset' => 'utf-8',
		];
        ...
        $this->email->from('mariaanjelin@gmail.com', 'Maria Anjelin');
        $this->email->to($this->input->post('email'));

        if($type == 'verify') {
            $this->email->subject('Account Verivication');
            $this->email->message('Clik this link to activeed :
            <a href="' . base_url() . 'auth/verify?email=' .
            $this->input->post('email') . '&token=' . urlencode($token) .
            '">Actived</a>');
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message('Clik this link to reset password :
	...
	public function forgotPassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($user) {
				...
				role="alert">Please check your email to reset password!</div');
				redirect('auth/forgotpassword');
			} else {
				$this->session->set_flashdata('message', '<div class="alert alert-danger"
				role="alert">Email is not registerd or activated!</div');
				redirect('auth/forgotpassword');
			}
		}
	}
